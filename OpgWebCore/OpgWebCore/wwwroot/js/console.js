...
function drawNewLineWithOuts(content) {
    ctx.font = outputFont;
    ctx.fillStyle = fontColor;
    var textOut = "  >>" + content;
    ctx.fillText(textOut, leftWindowMargin, cursor.y + lineHeight);
...
function keyDownHandler(e){
	...
	if((currentKey === 8 || currentKey === 'Backspace') && document.activeElement !== 'text') {
            ...
            if (cursor.x > promptWidth + hackForlen)
			{
				blotPrevChar();
	...
	if (currentKey == 13 || currentKey == 'Enter')
	{
        ...
        var cmdText = currentCmd.split(/\r|\n/g)[0]
        if (cmdText.length > 0)
		{
            allUserCmds.push(cmdText);
            // add for test
            SendMessage(cmdText, function (str) {
                hackForlen = 10;
                var result = JSON.parse(str);
                console.log(result);
                blotOutCursor();
                if (result.Type === "result") {
                    drawNewLineWithOuts(result.Value);
                    cursor.x = promptWidth - charWidth + hackForlen;
                    cursor.y += lineHeight;
                } 

                drawNewLine();
                // hack
                //promptWidth = promptWidth + 10;
                cursor.x = promptWidth - charWidth + hackForlen;
                cursor.y += lineHeight;
                currentCmd = "";
                //currentCmd.length = 0;
